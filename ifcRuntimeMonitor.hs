data Identifier = String deriving (Show,Eq)
...
data BaseValue = BoolVal Bool
               | NumVal Int
               | StringVal String
               | LamVal Identifier Expr deriving Show

data Value = Lab BaseValue Label deriving Show

data Expr = Var Identifier -- Changed from String to Identifier
          | NExp Int
          | BExp Bool
          | Plus Expr Expr
          | Conj Expr Expr
...
eval ec (Plus e1 e2) = let (Lab (NumVal v1) l1) = eval ec e1 in
                         let (Lab (NumVal v2) l2) = eval ec e2 in
                           Lab (NumVal (v1 + v2)) (join l1 l2)
